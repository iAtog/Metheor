    ...
    if(!args[0]){
      ...
      db.get(`SELECT * FROM banco WHERE id ="${msg.author.id}"`, (err, table) => {
        ...
        if(validar === msg.author.id) {
          ...
          db.run(`UPDATE banco SET money = ${dineroFinal} WHERE id = ${validar}`, (error) => {
      ...
      db.get(`SELECT * FROM banco WHERE id ="${msg.author.id}"`, (err, table) =>{
        ...
        if(validar === msg.author.id) {
          ...
          if(args[1] === "all") {
            db.run('UPDATE banco SET money = '+output.balance+' WHERE id = '+validar, (err) => {
              if(err)return console.error(err);
            });
          ...
          db.run(`UPDATE banco SET money = ${args[1]} WHERE id = ${validar}`, (err) => {
            if(err)return console.log(err);
          });
      ...
      if(!usuario) {
        ...
        db.get(`SELECT * FROM banco WHERE id ="${usuario.id}"`, (err, table) => {
          if(err)return console.error(err);
      ...
      db.get(`SELECT * FROM banco WHERE id = "${msg.author.id}"`, (err, table) => {
        ...
        db.run('INSERT INTO banco (id, money, tarjeta, expires, cvv) values ('+msg.author.id+', 0, '+generarTarjeta()+', '+generarExpiracion()+', '+generarCVV()+')', (err) => {
          if(err)return console.error(err);
        });
      ...
      db.run('CREATE TABLE IF NOT EXISTS banco (id TEXT, money INTEGER, tarjeta INTEGER, expires TEXT, cvv INTEGER)', (err) => {
        if(err)return console.error(err);
      });
      msg.channel.send("Tabla instalada correctamente");
    }else {
      msg.channel.send(":warning: Uso incorrecto.")
    }
    })();
  }
}
